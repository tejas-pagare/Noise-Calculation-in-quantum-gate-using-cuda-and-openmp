...
// Configuration
const int NUM_QUBITS = 10;         // Number of qubits
...
void simulateHEA(Qureg qureg, qreal noise, bool useTracked, std::ofstream& outFile, std::ofstream& traceFile, double& totalTime) {
    ...
    qreal avgError = totalError / (NUM_QUBITS * NUM_LAYERS);  // Average over all layers/qubits
...
int main() {
    ...
    for (size_t i = 0; i < NOISE_LEVELS.size(); ++i) {
...

        {
            ...
            simulateHEA(qureg, noise, true, out, traceFile, timeTrack);
            timeTrack = omp_get_wtime() - t1;
        }

        {
            double t2 = omp_get_wtime();
            simulateHEA(qureg, noise, false, out, traceFile, timeNoTrack);
            timeNoTrack = omp_get_wtime() - t2;
        }
